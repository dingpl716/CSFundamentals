...
/**
 * 给定一个长度为n的不重复数组，求所有可能的入栈出栈顺序。 也就是说把这个数组以不同的顺序入栈，再以不同的顺序出栈，求所有的出栈顺序。
 * 
 * 已犯错误： 没有进行逆操作，逆操作的地方有两个:
 * 
...
public class PushPopArray {
	 ...
	 * 核心思想:
	 * 
	 * 如果数组里面还有值，那么就肯定可以入栈. 再dfs，最后记得逆操作 如果stack里面还有值，那么久肯定可以出栈，再dfs，最后记得逆操作
	 * 
	 * @param n
	 * @param start
	 * @param buff
	 * @param stack
	 */
	private void dfs(int[] n, int start, ArrayList<Integer> buff, Stack<Integer> stack) {
