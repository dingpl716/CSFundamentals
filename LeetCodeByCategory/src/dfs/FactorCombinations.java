...
public class FactorCombinations {
	...
	private void dfs(int remaining, int start, List<List<Integer>> results, ArrayList<Integer> buffer) {
		...
		/*
		 * 假设上一轮的的remaining是16 那么这一轮的start，remaining的可能情况就是 2, 8 4, 4 8, 2 为了防止重复的情况 8，
		 * 2就不考虑了 所以我们要以上一轮的i作为这一轮的start，并且要让start小于remaining
	...
	/**
	 * 比上面要快的多的实现 执行过程如下： 初始输入为16
	 * 
	 * 第一层的输入16 start = 2, i = 2, remaining = 8 - > result add [2, 8]
	 * 
	 * 第二层remaining = 8 start = 2, i = 2, remaining = 4 - > result add [2, 2, 4]
	 * 
	 * 第三层 remaining = 4 start = 2, i = 2, remainging = 2 - > result add [2, 2, 2,
	 * 2]
	 * 
	 * 第三层 remaining = 4， i = 3, 大于remaining平方根，退出
	 * 
	 * 第二层remaining = 8 start = 2, i = 3, 大于remaining的平方根, 退出
	 * 
	 * 第一层remaining = 16 start = 2, i = 3不能整除, i = 4 -> result add[4, 4]
	 * 
	 * @param remaining
	 * @param start
	 * @param results
