...
public class MedianOfTwoSortedArrays {
	...
	/*
	 * 在A[start1, end1] 和 B[start2, end2]中找到第k小的数 If (m/2+n/2+1) > k && am/2 > bn/2
	 * , drop Section 2 If (m/2+n/2+1) > k && am/2 < bn/2 , drop Section 4 If
	 * (m/2+n/2+1) < k && am/2 > bn/2 , drop Section 3 If (m/2+n/2+1) < k && am/2 <
	 * bn/2 , drop Section 1 section 1, 3 小于待求的中位数，所以drop他们的时候要update k section 2，
	 * 4大于这个中位数，所以drop他们的时候不用update k
	 * 
	 * k is NOT 0 based, it is 1 based
	 */
	private double findKthNumberTwoSortedArrays2(int A[], int start1, int end1, int[] B, int start2, int end2, int k) {
		if (start1 > end1)
			return B[start2 + k - 1];
		if (start2 > end2)
			return A[start1 + k - 1];
	...
	private double findKthNumberTwoSortedArrays(int A[], int start, int end, int[] B, int k) {
		if (start > end)
